...
public class Ball {
    ...
    public Ball(float x, float y, float size){
        ...
        velocity = new Vector2(300, - 300);// speed and direction
    }
    ...
    public void update(float delta){
        ...
        if(bounds.y + bounds.height >= Gdx.graphics.getHeight()){
            velocity.y = -velocity.x; // reverse vertical direction when hitting the top
        }
    ...
    public void checkPaddleCollision(Paddle paddle){
        if(bounds.overlaps(paddle.getBounds())){
            // ball hits the paddle and then bounce upward
            ...
            float bounceAngle = normalizedRelativeIntersectionX * 60; // Will bounce max 60 degree
